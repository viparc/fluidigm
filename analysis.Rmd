...
#    transformation()
  ...
  control <- tmp %>%
    ...
    bind_rows() %>% 
    arrange(SamplingDay2) # for some reason we need this before calling boxplot()...
  points(jitter(control[[2]] - eps), control[[3]], col = col_control)
  boxplot2(control, -eps, col = col_control)
  
  treatment <- tmp %>%
    map(filter, Group == "Treatment") %>% 
    bind_rows() %>% 
    arrange(SamplingDay2) # for some reason we need this before calling boxplot()...
  points(jitter(treatment[[2]] + eps), treatment[[3]], col = col_treatment)
  boxplot2(treatment, eps, col = col_treatment)
...
legend("topright", legend = c("treatment", "control"),
       fill = adjustcolor(c("red", "blue"), .5), bty = "n")
```
